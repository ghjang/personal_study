...
결과적으로 위 문장은 `$2` 문자열 리터럴을 먼저 도출하고 이후에 `'$2' + 3`을 계산하게 되는데, 이때 문자열 리터럴이 숫자 리터럴로 암묵적 변환을 시도하면서 오류가 발생하게 된다.

'암묵적 변환' 규칙에 대해서는 아래 항목에서 좀더 설명한다.
...
리터럴의 타입 덤프, 산술, 비교 연산,... 결과 타입이 무엇인가?
3개 타입과 'NUMBER' 타입간의 연산결과가 무슨 타입인지 기록할 것.
우선순위와 타입 프로모션 규칙
