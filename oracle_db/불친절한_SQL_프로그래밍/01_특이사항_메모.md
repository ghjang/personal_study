...
*FIXME: 참고로 오라클 DB의 'PL/SQL'에서는 'BOOLEAN' 타입이 존재한다. 'BOOLEAN' 타입의 값을 받거나 리턴하는 함수를 정의할 수 있다. 하지만 그러한 함수를 'SQL'쪽에서 직접적으로 호출하는 것은 불가능한 것으로 보인다(?).*

*TODO: 'BOOLEAN' 타입을 다루는 'PL/SQL' 예시 코드를 추가할 것.*

*TODO: 'WHERE'절에서 의미적으로 존재하는 'TRUE, FALSE, UNKNOWN'의 타입을 확인 가능한 것인지 알아볼 것.*<br>
