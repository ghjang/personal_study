...
# WHERE 절의 조건(condition)은 행마다 평가되며, <<TRUE, FALSE, UNKNOWN>> 중 하나의 값을 반환한다. SELECT 문은 평가 결과가 TRUE인 행만 반환한다.

# 비교 조건(comparison condition)에서 NULL과의 직접적인 비교는 <<UNKNOWN>>으로 평가된다.
